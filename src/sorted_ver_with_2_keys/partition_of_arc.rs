...
#[derive(Debug, Clone)]
pub struct PartitionOfArc<TValue: EntityWith2StrKey> {
    pub(crate) rows: Vec<Arc<TValue>>,
}

impl<TValue: EntityWith2StrKey> EntityWithStrKey for PartitionOfArc<TValue> {
    fn get_key(&self) -> &str {
        self.rows.get(0).unwrap().get_primary_key()
    }
}

impl<TValue: EntityWith2StrKey> PartitionOfArc<TValue> {
    pub fn new(item: Arc<TValue>) -> Self {
        Self { rows: vec![item] }
    }
    ...
    pub fn insert_or_update<'s>(&'s mut self, key: &str) -> InsertOrUpdateEntry<'s, Arc<TValue>> {
        let insert_index = self
            .rows
    ...
    pub fn get_mut_or_create<'s>(
        &'s mut self,
        key: &str,
    ) -> GetMutOrCreateEntry<'s, Arc<TValue>> {
        let index = self
            .rows
            .binary_search_by(|itm| itm.get_secondary_key().cmp(key));
    ...
    pub fn get_mut(&mut self, key: &str) -> Option<&mut Arc<TValue>> {
        let result = self
            .rows
            .binary_search_by(|itm| itm.get_secondary_key().cmp(key));
    ...
    pub fn get_from_key_to_up(&self, secondary_key: &str) -> &[Arc<TValue>] {
        let result = self
            .rows
            .binary_search_by(|itm| itm.get_secondary_key().cmp(secondary_key));
    ...
    pub fn get_from_bottom_to_key(&self, secondary_key: &str) -> &[Arc<TValue>] {
...
mod tests {
    ...
    fn test_truncate_capacity() {
        let mut partition = PartitionOfArc::new(Arc::new(TestEntity {
            primary_key: "p".to_string(),
            secondary_key: "b".to_string(),
            value: 2,
        }));

        partition.insert_or_replace(Arc::new(TestEntity {
            primary_key: "p".to_string(),
